 ...
 * 
 * This is "Knox noticing something is wrong and fixing it herself."
 */
export function autoHealAlignment(
  ...
  log.push(`[Knox] Checking alignment quality (method: ${currentAlignment.method}, confidence: ${(currentAlignment.confidence * 100).toFixed(1)}%)...`);

  // First, validate the current alignment
  const originalDiag = validateAlignment(ecuDef);
  log.push(`[Knox] Health check: ${originalDiag.mapsInRange}/${originalDiag.totalMapsChecked} maps in range (${(originalDiag.healthScore * 100).toFixed(0)}% healthy)`);

  if (originalDiag.isHealthy && originalDiag.healthScore > 0.6) {
    log.push('[Knox] Alignment looks good — no intervention needed.');
    return {
      success: true,
  ...
  for (const issue of originalDiag.issues) {
    log.push(`[Knox] Issue detected: ${issue}`);
  }
  log.push('[Knox] Alignment looks suspicious. Trying alternative strategies...');
  ...
  log.push('[Knox] Strategy 1: Trying zero offset (A2L addresses = binary offsets)...');
  {
  ...
  log.push('[Knox] Strategy 2: Trying flipped byte order...');
  {
  ...
  log.push('[Knox] Strategy 4: Searching for calibration signatures in binary...');
  {
  ...
  log.push('[Knox] Strategy 5: Fine-grain sweep around map address clusters...');
  {
