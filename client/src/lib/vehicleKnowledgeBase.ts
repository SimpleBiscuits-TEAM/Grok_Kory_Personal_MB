...
const L5P_PLATFORM: VehiclePlatform = {
  ...
  commonDTCs: [
    ...
    { code: 'P0088', description: 'Fuel Rail/System Pressure Too High', severity: 'critical', commonCauses: ['Stuck fuel pressure regulator', 'Faulty PCV/FCA valve', 'ECM calibration issue'], diagnosticSteps: ['Monitor FPR / inlet metering current (mA), not PWM duty %', 'Check fuel pressure regulator operation', 'Verify ECM calibration version'] },
  ...
  specificPIDs: [
    ...
    { name: 'FPR current (inlet metering / PCV)', hpTunersName: 'Fuel Pressure Regulator DC', efiliveName: 'PCV_DC', units: 'mA', normalRange: '~400–1800 mA typical (calibration-specific)', description: 'ECM-commanded current to the high-pressure pump inlet metering valve (often labeled PCV); not a % duty cycle in physical units.', diagnosticRelevance: 'Rule of thumb: ~400 mA ≈ high opening (more flow toward rail), ~1800 mA ≈ low opening. Compare to desired vs actual rail pressure.' },
  ],
  diagnosticNotes: [
...
const LML_PLATFORM: VehiclePlatform = {
  ...
  commonDTCs: [
    ...
    { code: 'P0088', description: 'Fuel Rail/System Pressure Too High', severity: 'critical', commonCauses: ['Stuck FCA/PCV valve', 'Fuel pressure regulator failure', 'ECM calibration'], diagnosticSteps: ['Monitor FPR / PCV commanded current (mA)', 'Check fuel return line restriction', 'Verify ECM calibration'] },
...
const LLY_PLATFORM: VehiclePlatform = {
  ...
  specificPIDs: [
    ...
    { name: 'Boost Pressure', hpTunersName: 'Boost Pressure', efiliveName: 'GM.BOOST', units: 'PSI', normalRange: '0-30 PSI', description: 'Turbocharger boost pressure', diagnosticRelevance: 'LLY turbo mouthpiece restriction can limit boost. During turbo braking, boost stays elevated on decel — this is intentional if turbo braking is enabled in the tune.' },
    ...
    { name: 'Desired Turbo Vane Position', hpTunersName: 'Desired Turbo Vane Position', efiliveName: 'GM.TCVDES', units: '%', normalRange: '0-100%', description: 'ECM commanded turbo vane position. 0% = fully open, 99% = fully closed.', diagnosticRelevance: 'Vanes commanding 99% on decel with elevated desired boost = turbo braking active. Vanes commanding 99% without high desired boost = possible stuck vane or unison ring issue.' },
    { name: 'Turbo Vane Position (Actual)', hpTunersName: 'Turbo Vane Position', efiliveName: 'GM.TCVPOS', units: '%', normalRange: '0-100%', description: 'Actual turbo vane position. Compare to desired for response and sticking.', diagnosticRelevance: 'Slow response (>5% deviation from desired for >1 sec) indicates sticking vanes or unison ring wear. On decel, vanes at 90%+ with boost still elevated = turbo braking surge.' },
    { name: 'Desired Turbo Vane Boost Level', hpTunersName: 'Desired Turbo Vane Boost Level', efiliveName: 'GM.TCVSDBST', units: 'kPa', normalRange: '100-255 kPa', description: 'ECM desired boost level for VGT control.', diagnosticRelevance: 'Key turbo braking indicator: if desired boost is elevated (>150 kPa) while throttle is 0%, the ECU is intentionally commanding turbo braking. This is the smoking gun for turbo braking confirmation.' },
    { name: 'DSP5 Switch Voltage', hpTunersName: 'DSP5 Switch Voltage', efiliveName: 'GM.DSP5_VOLT', units: 'V', normalRange: '0-5V', description: 'EFI Live DSP5 tune selector switch voltage', diagnosticRelevance: 'Identifies which DSP5 tune level is active. Different voltage ranges correspond to different tune positions.' },
    { name: 'DSP5 Tune Level', hpTunersName: 'DSP5 Tune (LB7-LLY)', efiliveName: 'GM.DSP5_TUNE', units: '', normalRange: 'DSP5.1-DSP5.4', description: 'Active DSP5 tune selection. DSP5 is an EFI Live custom operating system that allows multiple tunes via a selector switch.', diagnosticRelevance: 'Tune level mapping: Non-DSP=Selector 1, DSP5.1=Selector 2, DSP5.2=Selector 3, DSP5.3=Selector 4, DSP5.4=Selector 5. Lower levels (DSP5.1-DSP5.2) are typically tow tunes with turbo braking. Higher levels are performance tunes.' },
  ],
  diagnosticNotes: [
    ...
    'TURBO SURGE DIAGNOSTICS: Turbo surge on decel is commonly caused by turbo braking in tow tunes (especially DSP5.1 and DSP5.2 on EFI Live DSP5 systems). The diagnostic signature is: (1) Throttle at 0% (decel), (2) Desired boost elevated well above atmospheric (>150 kPa) — this is the ECU commanding turbo braking, (3) Desired vane position at 99% (fully closed) — the vanes are trying to achieve that desired boost, (4) Actual boost stays high (>120 kPa / 17+ psi) despite 0% throttle. The desired boost being high on decel is the smoking gun — it proves the ECU is intentionally commanding turbo braking, not a stuck vane or sensor issue.',
    'DSP5 TUNE SYSTEM: EFI Live DSP5 is a custom operating system that allows up to 5 tunes via a selector switch. Tune level mapping: Non-DSP = Tune selector 1, DSP5.1 = Tune selector 2, DSP5.2 = Tune selector 3, DSP5.3 = Tune selector 4, DSP5.4 = Tune selector 5. Tow tunes are generally in the lower levels (DSP5.1 or DSP5.2). Higher levels are typically performance/race tunes.',
    'TURBO BRAKING: When turbo braking is enabled in a tune, the ECU commands the VGT vanes to close (99%) on decel to create exhaust restriction. This creates engine braking effect useful for towing. The side effect is turbo surge — the turbo continues to spool from residual exhaust energy, keeping boost elevated when the driver expects deceleration. Fix: Switch to a non-tow tune, or have the tuner reduce turbo braking aggressiveness by lowering the decel vane position target from 99% to 60-70%.',
    'TURBO BRAKING vs STUCK VANES: Key differentiation — if desired boost is elevated on decel AND desired vane is 99%, it is turbo braking (intentional). If desired vane is low but actual vane is stuck high, it is a mechanical issue (unison ring, carbon buildup, or actuator failure).',
  ],
};
...
const LB7_PLATFORM: VehiclePlatform = {
  ...
  diagnosticNotes: [
    ...
    'Turbo shaft speed concern: a boost leak forces the turbo to spin harder trying to compensate for lost pressure, which can overspeed the turbo. A tight converter stall alone does not overspeed the turbo. However, if both a tight stall AND a boost leak are present, the combination is particularly hard on the turbo — low RPM from the stall plus extra shaft speed from the leak. This is especially concerning with aftermarket turbos.',
