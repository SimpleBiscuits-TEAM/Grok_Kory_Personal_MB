...
export const ECU_PARAMETERS: Record<string, EcuParameter> = {
  ...
  fuelRailActual: {
    ...
    unit: 'psi',
    description: 'Sensed absolute fuel rail pressure from the high-pressure fuel rail sensor (FRP sensor). Used by the ECM for closed-loop fuel pressure control via the PCV solenoid. High-pressure fuel pump system (HP4 on L5P, CP4.2 on LML, CP3 on LBZ/LMM/LLY, CP3.3 on LB7). Sensitive to fuel quality and filter condition.',
    normalMin: 3000,
    normalMax: 26000,
    warnMin: 1500,
    warnMax: 28000,
  ...
  fuelRailDesired: {
    ...
    unit: 'psi',
    description: 'ECM-commanded fuel rail pressure target. Calculated based on engine load, RPM, and fuel quantity demand tables. The high-pressure pump PCV (Pressure Control Valve) current is modulated to achieve this target.',
    normalMin: 3000,
    normalMax: 26000,
    category: 'fuel_rail',
  },
...
export const DTC_DEFINITIONS: DtcDefinition[] = [
  ...
  // ── FUEL SYSTEM ─────────────────────────────────────────────────────────────
  {
    ...
    causes: [
      'Contaminated or bad fuel quality (high-pressure pump is sensitive to clean, lubricated diesel fuel)',
      'Clogged fuel filter (replace every 15–20k miles)',
      'Weak or failing lift pump',
      'Failing high-pressure pump (HP4 on L5P, CP4.2 on LML, CP3 on older platforms)',
  ...
  },
  {
    ...
    causes: [
      'Worn high-pressure pump (HP4 on L5P, CP4.2 on LML, CP3 on older platforms)',
      'Stuck fuel pressure regulator',
      'Severe fuel contamination',
    ],
    remedies: [
      'Evaluate high-pressure pump output (HP4 on L5P, CP4.2 on LML, CP3 on older platforms)',
...
export const L5P_SPECS = {
  ...
  subsystems: {
    ...
    INJC: 'Fuel Injection Control — Manages injector pulse width, timing, and pilot/main/post injection events. The ECM commands up to 5 injection events per combustion cycle (2 pilot, 1 main, 2 post). Pilot injections reduce combustion noise; post injections support DPF regen. Injector balance rates (IBRT) indicate individual injector health — values >4 mg/stroke suggest wear or fouling.',
    LPFP: 'Low-Pressure Fuel System — Lift pump and fuel filter assembly (feeds the HP4 on L5P, CP4 on LML, CP3 on LBZ/LMM). The lift pump provides ~8-10 psi to the high-pressure pump inlet. Fuel filter differential pressure sensor monitors restriction. Filter life typically 15,000-20,000 miles. Air intrusion at filter housing seals is a common cause of hard starts and P0087.',
    FQUL: 'Fuel Quality Monitoring — The ECM infers fuel quality from combustion characteristics, injection timing corrections, and crankshaft acceleration patterns. Poor fuel quality (water contamination, low cetane, biodiesel blends >B20) causes increased pilot injection quantity, rough idle, and elevated combustion noise.',
    // ── Turbo & Air System ──
    ...
    ICLR: 'Intercooler System — Charge air cooler (CAC) reduces intake air temperature after turbo compression. The CAC efficiency is monitored via intake air temp sensors pre/post cooler. Efficiency drops below 80% indicate internal restriction or external blockage. Condensation in the CAC during humid conditions can cause momentary white smoke on acceleration.',
    THTL: 'Electronic Throttle Control — Drive-by-wire throttle body with dual position sensors. The ECM uses the throttle for intake air management, EGR flow control, and engine braking (exhaust brake). Throttle position does NOT directly correlate to driver pedal position — the ECM interpolates based on torque demand, emissions targets, and turbo lag compensation.',
    // ── EGR System ──
    EGRV: 'EGR Valve Control — Exhaust Gas Recirculation valve with position feedback sensor. The ECM commands EGR flow to reduce NOx emissions by lowering combustion temperatures. EGR cooler bypass valve routes exhaust around the cooler during warm-up. Common failure: EGR valve sticking from carbon buildup, causing rough idle and P0401/P0402.',
    EGRC: 'EGR Cooler System — Liquid-cooled EGR heat exchanger that reduces exhaust gas temperature before recirculation. Cooler efficiency monitored via inlet/outlet temperature sensors. Cracked coolers leak coolant into the exhaust, causing white smoke and coolant loss. Bypass valve failure keeps cooler active during cold start, increasing warm-up time.',
    // ── Exhaust & Emissions ──
    ...
    NOXS: 'NOx Sensor Monitoring — Dual NOx sensors (upstream and downstream of SCR catalyst) measure nitrogen oxide concentration. Upstream sensor provides feed-forward DEF dosing; downstream sensor verifies SCR conversion efficiency. Sensor heater circuits monitored for open/short. Common failure mode: sensor reading stuck or erratic after 80-100k miles.',
    CATM: 'Catalytic Converter Monitoring — Diesel Oxidation Catalyst (DOC) upstream of DPF converts CO and HC to CO2 and H2O. DOC efficiency monitored via temperature rise across the catalyst. Insufficient temperature rise indicates catalyst degradation. DOC also raises exhaust temperature for DPF regeneration initiation.',
    // ── Transmission ──
    ...
    TCCR: 'Torque Converter Clutch Control — The TCM manages TCC apply pressure via a pulse-width-modulated solenoid. States: ImmediateOff (unlocked), ControlledOff (releasing), ControlledHyst (hysteresis zone), ControlledOn (controlled apply with intentional slip during acceleration), and FullLock (zero slip target). The 10L90 uses "ControlledOn" during acceleration with 200-700 RPM of intentional slip for torque multiplication — this is normal TCM behavior, not a fault.',
    SHFT: 'Shift Control — The 10L90 (Gen 1) and 10L1000 (Gen 2) use clutch-to-clutch shifting with 6 multi-plate clutch packs. Shift timing is adaptive — the TCM learns optimal apply/release pressures. Harsh shifts indicate adaptive values need reset or fluid degradation. Shift flare (RPM spike between gears) indicates clutch pack wear.',
    TRNF: 'Transmission Fluid Monitoring — ATF temperature monitored via internal sensor. Normal operating range: 160-200°F. Warning at 250°F, critical at 280°F. Fluid life calculated from temperature exposure history. The 10L90 requires Mobil 1 Synthetic LV ATF HP; incorrect fluid causes shift quality issues.',
    TRNG: 'Transmission Gear Ratio Monitoring — The TCM monitors actual gear ratio (input speed / output speed) vs commanded ratio. Ratio errors indicate clutch slip, band wear, or planetary gear damage. The 10L90 has 10 forward gear ratios from 4.70:1 (1st) to 0.64:1 (10th).',
    // ── Cooling System ──
    COOL: 'Engine Cooling System — Dual thermostat system with ECM-controlled coolant flow. Primary thermostat opens at ~190°F, secondary at ~210°F. Electric auxiliary coolant pump runs during DPF regen and turbo cool-down. Coolant temp sensor (ECT) provides feedback for fuel injection timing, idle speed, and glow plug control. Normal operating range: 185-210°F.',
    OILM: 'Engine Oil Monitoring — Oil life monitoring system calculates remaining oil life from operating conditions (RPM, load, temperature, fuel dilution model). Oil pressure sensor monitors lubrication system. Low oil pressure at idle (<15 psi warm) indicates bearing wear or oil pump degradation. Oil temperature typically 20-40°F above coolant temperature.',
    // ── Electrical & Sensors ──
    GLWP: 'Glow Plug System — 8 ceramic glow plugs with individual ECM-controlled drivers. Pre-heat cycle: 2-10 seconds depending on ambient temperature. Post-heat continues up to 3 minutes after start for emissions reduction. Glow plug resistance monitored for open/short circuit detection. Failed glow plugs cause hard cold starts and white smoke.',
    BATT: 'Battery & Charging System — Dual battery system (2017+ HD trucks). ECM monitors battery voltage, alternator output, and parasitic draw. Intelligent Battery Sensor (IBS) tracks state of charge and state of health. Low voltage (<11.8V) affects ECM/TCM operation and can cause false DTCs.',
    CANS: 'CAN Bus Communication — High-speed CAN (500 kbps) for powertrain (ECM, TCM, ABS). Medium-speed CAN for body (BCM, IPC, HVAC). The OBD-II port accesses diagnostic CAN via the gateway module. 2024+ E42 ECM requires directed addressing (ATSH 7E0/7E1) for Mode 22 extended PIDs.',
    CRKS: 'Crankshaft Position System — 58-tooth reluctor wheel with 2-tooth gap for TDC reference. The ECM uses crankshaft acceleration patterns for misfire detection, combustion quality monitoring, and injector balance rate calculation. Camshaft position sensor provides cylinder identification for sequential injection.',
    // ── Engine Protection ──
    ENGP: 'Engine Protection System — Multi-level protection strategy: Level 1 = warning lamp + reduced power. Level 2 = severe power reduction + speed limiting. Level 3 = engine shutdown. Triggers include: over-temperature (coolant >250°F, EGT >1500°F), low oil pressure, over-speed (>4200 RPM), and turbo over-boost (>55 psi).',
    EXBK: 'Exhaust Brake System — Uses VGT vane closure and intake throttle restriction to create engine braking. The ECM coordinates VGT position, throttle position, and fuel injection cutoff. Exhaust brake effectiveness increases with RPM. Automatic activation available via cruise control settings on 2017+ models.',
    DIAG: 'On-Board Diagnostics (OBD-II) — Continuous and non-continuous monitors for all emissions-related systems. Mode 01: live data. Mode 02: freeze frame. Mode 03/07: stored/pending DTCs. Mode 06: test results. Mode 09: VIN and calibration IDs. Readiness monitors must complete before emissions testing.',
  },
};
