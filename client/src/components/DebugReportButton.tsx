...
const STATUS_INFO: Record<string, { label: string; color: string; icon: typeof Bug }> = {
  ...
  analyzing: { label: 'Knox Analyzing...', color: 'text-yellow-400', icon: Loader2 },
...
export default function DebugReportButton() {
  ...
  const submitReport = trpc.debug.submitReport.useMutation({
    onSuccess: (data) => {
      toast.success(`Bug report #${data.sessionId} submitted! Knox is analyzing...`);
      setShowForm(false);
      resetForm();
  ...
  const submitRetest = trpc.debug.submitRetest.useMutation({
    onSuccess: (data) => {
      toast.success(data.status === 'confirmed_fixed' ? 'Marked as fixed!' : 'Feedback submitted — Knox will re-analyze');
      sessionsQuery.refetch();
    },
    onError: (e) => toast.error(e.message),
  });
                      ...
                      <div className="border-t border-zinc-800 p-2.5 space-y-2">
                        ...
                        {s.rootCause && (
                          <div className="bg-zinc-900/50 rounded p-2 border-l-2 border-purple-500">
                            <p className="text-[10px] text-purple-400 font-[Share_Tech_Mono] uppercase">Knox's Analysis</p>
