...
class PCANBridge:
    ...
    async def handle_message(self, msg: dict) -> Optional[dict]:
        ...
        # ─── Raw CAN recv (no TX) — ISO-TP CF2+ after FC was sent via can_send ───
        elif msg_type == "can_recv":
            if not self.can_initialized:
                success = await self._ensure_can_bus()
                if not success:
                    return {
            ...
            try:
                response = await asyncio.wait_for(
                    self.protocol._response_queue.get(),
                    timeout=timeout_sec,
                )
                return {
                    "type": "can_frame",
                    ...
                    "timestamp": response.timestamp or time.time(),
                }
            except asyncio.TimeoutError:
                return {
                    "type": "error",
                    "id": req_id,
                    "message": "Timeout waiting for CAN frame (can_recv)",
                }
